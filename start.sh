...
install_datapacks() {
    if [ -f /datapack_urls.txt ]; then
        ...
        done < /datapack_urls.txt
        echo "Data packs installed."
        rm /datapack_urls.txt
        echo "datapack_urls.txt deleted."
    else
        echo "No datapack_urls.txt found. Skipping data pack installation."
    fi
}
